...
gradlePlugin {
    plugins {
        create("DeploymentPlugin") {
            id = "laplacian-arch.model.deployment"
            implementationClass = "laplacian_arch.deployment.gradle.DeploymentPlugin"
        }
    }
}
...
publishing {
    ...
    publications {
        create<MavenPublication>("mavenJava") {
            artifactId = "laplacian-arch.model.deployment"
            artifact(sourcesJar)
        }
        create<MavenPublication>("model") {
            artifactId = "laplacian-arch.model-def.deployment"
            artifact(modelJar)
        }
    }
}